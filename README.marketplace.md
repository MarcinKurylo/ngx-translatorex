...
- **Any MCP agent (Claude, …)** — the same operations ship as a standalone [Model Context Protocol](https://modelcontextprotocol.io) server, so agents outside VS Code (Claude Desktop, Claude Code) can drive the identical workflow. See [`mcp/README.md`](https://github.com/MarcinKurylo/ngx-translatorex/blob/main/mcp/README.md).
